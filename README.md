...
- 两栏版式：左侧固定边栏（桌面）/ 抽屉（移动），右侧阅读区
- 边栏含：
  - 实时搜索（200ms 防抖）—— 经文引用 `诗 23:1` 出绿色直跳卡片；关键词出最多 30 条命中卡片，可滚动；高亮搜索词
  - 简体 / 繁体 切换（繁体暂未提供，预留入口）
  - 全部 / 旧约 / 新约 过滤
  - 66 卷书卷网格，当前书卷绿色高亮
- 阅读区：URL 形如 `/read/GEN/1/`，每节有 `#vN` 锚点便于分享；当前节绿色高亮 + 左侧竖线
- 客户端导航 + JS 驱动节高亮：点击侧栏结果右侧切换章节，侧栏完全不刷新，搜索结果保留
- 右侧正文同步高亮搜索词（黄色 mark，与侧栏卡片一致）
- 键盘：`←` / `→` 翻章
